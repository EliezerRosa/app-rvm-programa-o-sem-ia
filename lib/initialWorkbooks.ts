import { Workbook } from '../types';
import { generateUUID } from './utils';

const validWorkbookPdfBase64 = "";

export const initialWorkbooks: Workbook[] = [
  {
    id: generateUUID(),
    name: 'Nossa Vida e Ministério Cristão - Nov/Dez 2024',
    fileData: validWorkbookPdfBase64,
    uploadDate: new Date('2024-11-01T10:00:00Z').toISOString()
  },
  {
    id: generateUUID(),
    name: 'Nossa Vida e Ministério Cristão - Jan/Fev 2025',
    fileData: validWorkbookPdfBase64,
    uploadDate: new Date('2025-01-01T10:00:00Z').toISOString()
  }
];
